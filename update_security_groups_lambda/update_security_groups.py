...
def get_ranges_for_service(ranges, service, subset):
    ...
    for prefix in ranges["prefixes"]:
        if prefix["service"] == service and (
            (subset == prefix["region"] and subset == "GLOBAL")
...
def update_security_groups(new_ranges, rangeType):
    ...
    for curGroup in allSGs:
        ...
        rangesToUpdate = get_security_groups_for_update(client, tagToFind)
        msg = "tagged Name: {}, Protocol: {} to update".format(
            tagToFind["Name"], curGroup
        )
        logging.info("Found {} groups {}".format(str(len(rangesToUpdate)), msg))

        if len(rangesToUpdate) == 0:
            result.append("No groups {}".format(msg))
            logging.warning("No groups {}".format(msg))
        else:
            for rangeToUpdate in rangesToUpdate:
                if update_security_group(
                    client, rangeToUpdate, new_ranges, INGRESS_PORTS[curGroup]
                ):
                    result.append(
                        "Security Group {} updated.".format(rangeToUpdate["GroupId"])
                    )
                else:
                    result.append(
                        "Security Group {} unchanged.".format(rangeToUpdate["GroupId"])
                    )
...
def update_security_group(client, group, new_ranges, port):
    ...
    logging.debug(
        (group["GroupId"] + ": Added " + str(added) + ", Revoked " + str(removed))
    )
